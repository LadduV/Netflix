...
export default React.memo(function MovieSlider  ({data, title})  { 
  ...
  return (
   <SlideContainer controlVisibility={controlVisibility} onMouseEnter={()=>setControlVisibility(true)} onMouseLeave={()=>setControlVisibility(false)}>
    <h1>{title}</h1>
    ...
    <div className={`slider-action right  ${!controlVisibility ? 'none': ''}`}>
            ...
            <AiOutlineRight onClick={()=>handleDirection('left')}/>
        </div>
    </div>
